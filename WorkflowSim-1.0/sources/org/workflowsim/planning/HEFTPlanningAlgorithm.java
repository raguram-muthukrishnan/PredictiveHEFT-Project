...
// Trivial case: Start after the latest task scheduled
     ...
     * Returns the computation cost (time in seconds) to execute a task on a VM.

     * Default implementation: task length divided by VM MIPS.

     *

     * @param task The task to compute cost for.

     * @param vm The VM to compute cost for.

     * @return Computation cost in seconds.

     */

    protected double getComputationCost(Task task, org.cloudbus.cloudsim.Vm vm) {

        if (vm instanceof CondorVM) {

            CondorVM condorVm = (CondorVM) vm;

            if (condorVm.getNumberOfPes() < task.getNumberOfPes()) {

                return Double.MAX_VALUE;

            } else {

                return task.getCloudletTotalLength() / condorVm.getMips();

            }

        } else {

// Fallback for non-CondorVM VMs
